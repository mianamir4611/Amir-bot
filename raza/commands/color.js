module.exports = {
  config: {
    name: 'color',
    aliases: ['theme', 'setcolor', 'groupcolor', 'grouptheme'],
    description: 'Change group chat color/theme',
  ...
  async run({ api, event, args, send }) {
    ...
    if (!args.length) {
      return send.reply(`🎨 Available Theme Colors:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Special Themes:
Aqua, Mango, Berry, Citrus, Candy, Earth, Support,
Music, Pride, DoctorStrange, LoFi, Sky,
LunarNewYear, Celebration, Chill, StrangerThings,
Dune, Care, Astrology, Birthday, Cottagecore, Ocean,
Love, TieDye, Monochrome, Rocket, Unicorn, Tropical,
Maple, Sushi, Shadow, Rose, Lavender, Tulip,
Classic, Peach, Honey, Kiwi, Grape, NonBinary,
Transgender, TaylorSwift, Autumn, Cyberpunk2077,
& many more!
...
Example: .color blue`);
    ...
    if (!colorID) {
      return send.reply(`❌ Color "${colorName}" not found.\n\nTry .color for a list of available colors.`);
    }

    try {
      await api.changeThreadColor(colorID, threadID);
      return send.reply(`✅ Group theme changed to **${colorName}**! 🎨`);
    } catch (error) {
      return send.reply(`❌ Failed to change color: ${error.message}`);
    }
  }
};
